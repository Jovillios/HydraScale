...
class HydraDataLoader(DataLoader):
    def __init__(
        ...
        self.sampler = DistributedSampler(self.tokenized_dataset, num_replicas=world_size, rank=rank)
        super().__init__(self.tokenized_dataset, collate_fn=self.collate_batch, batch_size=self.batch_size, pin_memory=True, num_workers=num_workers, shuffle=False, sampler=self.sampler)

    def tokenize_group_text(self, examples, tokenize, seq_len):
        tokenized_text_batch = tokenize(examples["content"], return_attention_mask=False, return_tensors="np")
        ...
        result = {"input_ids": concatenated_tokens["input_ids"][:total_length].reshape(-1, seq_len)}
        return result
...
if __name__ == "__main__":
    dataloader = HydraDataLoader(
        seq_len=128, batch_size=4, dataset_name="ProCreations/Ultra-FineWeb-EDU", tokenizer_name="HuggingFaceTB/SmolLM-360M-Instruct", num_workers=0, subset=1000, world_size=1, rank=0, split="train"
    )
